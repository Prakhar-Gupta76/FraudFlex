...
export default function App() {
  ...
  async function reviewAlert(
    analystId: string,
    outcome: "confirmed_fraud" | "legitimate" | "needs_further_investigation",
    notes: string,
  ) {
...
function AlertInvestigation({ detail, onReview }: { detail: AlertDetail; onReview: (analyst: string, outcome: "confirmed_fraud" | "legitimate" | "needs_further_investigation", notes: string) => Promise<void> }) {
  return <div className="investigation"><p className="eyebrow">Alert investigation</p><h2>{shortId(detail.alert_id)}</h2><div className="hero-score danger"><div><span>Risk score</span><strong>{detail.final_score}<small>/100</small></strong></div><StatusBadge status={detail.status} /></div><InfoGrid items={[["Transaction", shortId(detail.transaction_id)], ["Customer", shortId(detail.customer_id)], ["Action", labelAction(detail.recommended_action)], ["Assigned", detail.assigned_to ?? "Unassigned"]]} /><Explanation detail={detail} />{detail.review_history.length > 0 && <section className="drawer-section"><h3><Clock3 size={16} /> Review history</h3><div className="review-timeline">{detail.review_history.map((review) => <div key={review.review_id}><i /><div><strong>{labelOutcome(review.outcome)}</strong><span>{review.analyst_id} · {formatDate(review.reviewed_at)}</span><p>{review.notes || "No notes recorded."}</p><small>{review.previous_status} → {review.new_status}</small></div></div>)}</div></section>}{detail.status === "resolved" ? <div className="resolved-card"><CheckCircle2 /><div><strong>Review completed</strong><span>{labelOutcome(detail.review_outcome)} by {detail.analyst_id}</span><p>{detail.review_notes}</p></div></div> : <ReviewForm onSubmit={onReview} />}</div>;
}
...
function ReviewForm({ onSubmit }: { onSubmit: (analyst: string, outcome: "confirmed_fraud" | "legitimate" | "needs_further_investigation", notes: string) => Promise<void> }) {
  const [analyst, setAnalyst] = useState("");
  const [outcome, setOutcome] = useState<"confirmed_fraud" | "legitimate" | "needs_further_investigation">("confirmed_fraud");
  ...
  return <form className="review-form" onSubmit={submit}><h3><ShieldCheck size={17} /> Record analyst decision</h3><label>Analyst ID<input required value={analyst} onChange={(event) => setAnalyst(event.target.value)} placeholder="e.g. analyst-104" /></label><label>Outcome<select value={outcome} onChange={(event) => setOutcome(event.target.value as typeof outcome)}><option value="confirmed_fraud">Confirmed fraud</option><option value="legitimate">Legitimate transaction</option><option value="needs_further_investigation">Needs further investigation</option></select></label><label>Investigation notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Record observed evidence and customer contact…" rows={4} /></label>{error && <p className="form-error">{error}</p>}<button className="primary-button" disabled={saving}>{saving ? <LoaderCircle className="spin" size={16} /> : <Check size={16} />} Record review</button><small>Interim and final reviews remain in the audit history.</small></form>;
}
